[@react.component]
let make = (~items: Belt.Map.String.t(Reshape.Type.collideBox)) => {
      ...
      <React.Fragment key>
        ...
        <div className="grid grid-cols-12 gap-1 mb-3">
          ...
          <div className="col-span-8">
            <Vector value={collideBox.size} onChange={_ => {()}} />
          </div>
          <div className="col-span-4"> {React.string("position")} </div>
          <div className="col-span-8">
            <Vector value={collideBox.position} onChange={_ => {()}} />
          </div>
          {Belt.List.length(collideBox.collisions) > 0
             ? <div> {React.string("Collisions:")} </div> : React.null}
          {Belt.List.map(
             collideBox.collisions, (collisionType: Reshape.Type.collideType) =>
